...
class Picker:
    ...
    def get_maps(self, score, micro_img, particle_windows, non_noise_windows):
        ...
        Args:
            ...
            non_noise_windows: Number of windows that contain neither noise
                nor particles.
        """
        ...
        begin_row_idx = y * qs
        end_row_idx = begin_row_idx + self.query_size
        end_row_idx = np.where(end_row_idx > bw_mask_p.shape[0],
                               bw_mask_p.shape[0], end_row_idx)

        begin_col_idx = x * qs
        end_col_idx = begin_col_idx + self.query_size
        end_col_idx = np.where(end_col_idx > bw_mask_p.shape[1],
                               bw_mask_p.shape[1], end_col_idx)

        for j in range(particles):
            bw_mask_p[begin_row_idx[j]:end_row_idx[j],
                      begin_col_idx[j]:end_col_idx[j]] = 1
